        ...
        @if($count==1)
            <li style="width: 100%;" class="media first stand align-items-stretch">
                <div class="media-left-first mr-3">
                    ...
                    <a class="media-thumb" href="/song/{{$song->id}}/{!! Str::slug($song->name,'-') !!}.html" title="{{$song->name}}"><img src="{{$song->thumb}}" width="90px" height="90px"></a>
                </div>
                <div class="media-body d-flex flex-column ">
                    <h6 class="mt-0 mb-0"><a href="/song/{{$song->id}}/{!! Str::slug($song->name,'-') !!}.html" title="{!! $song->lyric !!}">{{ucwords($song->name)}}</a></h6>
        ...
        @else
            <li style="width: 100%;" class="media now up align-items-stretch">
                <div class="media-left mr-3">
                    ...
                    @endif
                    <a href="/song/{{$song->id}}/{!! Str::slug($song->name,'-') !!}.html" title="{{$song->name}}"><img src="{{$song->thumb}}" width="40px" height="40px"></a>
                </div>
                <div class="media-body d-flex flex-column justify-content-between">
                    <h6 class="media-title mt-0 mb-0"><a href="/song/{{$song->id}}/{!! Str::slug($song->name,'-') !!}.html" title="">{{ucwords($song->name)}}</a></h6>
