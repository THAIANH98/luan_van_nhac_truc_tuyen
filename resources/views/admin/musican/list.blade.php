<table class="table">
    ...
    @foreach($musicans as $musican)
        <tr>
            <td>
                <input type="checkbox" name="del_id[]" onclick="showbutton()" style="height: 20px;width: 20px"  value="{{$musican->id}}" >
            </td>
            <td> {{ $counter }} </td>
            <td style="text-align: left; width: auto">{{$musican->name}}</td>
            <td>
                <img src="{!! $musican->avatar !!}" width="100px">
            </td>
            <td> <div id="parent_active_{{$musican->id}}"></div>{!!  \App\Http\Helper\Helper::active($musican->active,$musican->id,'/admin/musican/change/'.$musican->id) !!}</td>
            @if($musican->birthday != null )
                <td>{{$musican->birthday}}</td>
            @else
                <td>Chưa có ngày sinh</td>
            @endif
            <td>
                <a class="btn btn-primary btn-sm" href="/admin/musican/edit/{{ $musican->id}}" >
...
<script>
    ...
    function delid(){
        if(confirm('Dữ liệu xóa không thể khôi phục. Bạn có muốn xóa không?')){
            ...
            for($i=0 ; $i <$iddel.length;$i++){
                if($iddel[$i].checked===true){
                    removeRow($iddel[$i].value,'/admin/musican/destroy');
                }
            }
        }
    }
