...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            @include('admin.alert')
            <h3 style="color: #0689ba;margin-bottom: 5%">Sửa Playlist</h3>
            ...
            <form method="post">
                <div class="box_right_upload form-row">
                    ...
                    <div class="form-group col-12">
                        ...
                        <input type="text" class="form-control" id="name" value="{{$playlist->name}}" name="name" placeholder="Nhập tên playlist">
                    </div>
                    ...
                    <div class="form-group col-6">
                        ...
                        <select class="form-control" name="menu_id" id="menu_id">
                            @foreach($menus as $menu)
                                <option {{$playlist->menu_id == $menu->id ? 'selected':''}} value="{{$menu->id}}">{{$menu->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-6">
                        ...
                        <select class="form-control" name="category_id" id="category_id" >
                            @foreach($category as $cate)
                                <option {{$playlist->category_id == $cate->id ? 'selected':''}} value="{{$cate->id}}">{{$cate->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-12">
                    ...
                    <div class="form-group col-12">
                        ...
                        <div class="form-control pl-4 " style="  width: 100%;min-height: 50px;height: auto">
                            <ul id="danhsachchon" style="   height: 200px;
                                                                  overflow-y: scroll;
                                                                  scrollbar-color: #656262;
                                                                  scrollbar-width: thin;" >
                                @foreach($playlist->playlist_song as $song)
                                    <li id="song_id{{$song->id}}">
                                        {{$song->name}}
                                        <button id="del_song_id{{$song->id}}" onclick="removeli('{{$song->id}}')" class="btn btn-danger" style="right: 0px; float: right;">Xóa</button>
                                        <input name="song_id[]" id="song_input{{$song->id}}" style="display: none;">
                                        <hr>
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    <div class="text-center col-12">
                        <button type="submit" onclick="reloadpage()" id="btn-upload" class="btn btn-primary btn-upload">Cập nhật</button>
                    </div>
                </div>
                @csrf
            </form>
