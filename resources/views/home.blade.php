...
@section('content')
    ...
    <div class="row">
        <div class="col-md-9">
                        ...
                        <div class="col">
                            <div class="card card1">
                                <a href="/song/{{$song->id}}/{!! Str::slug($song->name,'-') !!}.html" title="{{$song->name}}">
                                    <div id="{{$song->id}}" class="card-header" style="background-image: url({{$song->thumb}});">
                                    <span style="visibility: hidden;background:url(https://data.chiasenhac.com/imgs/icon.png)  left -28px no-repeat;position: absolute;
                                                                ...
                                                                width: 48px;
                                                                height: 48px;
                                                                display: block;" id="icon{{$song->id}}"></span>
                                    </div>
                                </a>
                                <div class="card-body">
                                    <h3 class="card-title"><a href="/song/{{$song->id}}/{!! Str::slug($song->name,'-') !!}.html" title="{{$song->name}}">{{$song->name}}</a></h3>
                                    @foreach($song->song_singer as $singer)
                                        <p class="card-text"><a href="/singer/{{$singer->id}}/{!! Str::slug($singer->name,'-') !!}.html">{{$singer->name}};</a></p>
                                    @endforeach
                                </div>
                            </div>
                        </div>
            ...
            <div>
                <br><h2 style="color: #007efc; ">Playlist mới nhất</h2><br>
            </div>
                        ...
                        <div class="col">
                            <div class="card card1">
                                ...
                                <div class="card-body">
                                    <h3 class="card-title"><a href="/playlist/{{$playlist->id}}/{!! Str::slug($playlist->name,'-') !!}.html" title="{{$playlist->name}}">{{$playlist->name}}</a></h3>
                                    {{--                                    @foreach($song->song_singer as $singer)--}}
                                    {{--                                        <p class="card-text"><a href="https://chiasenhac.vn/tim-kiem?q=Vượng Tử Tiểu Kiều&amp;filter=ca-si">{{$singer->name}};</a></p>--}}
                                    {{--                                    @endforeach--}}
                                </div>
                            </div>
                        </div>
